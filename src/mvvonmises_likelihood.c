...
/*****
 * CHANGING TO REMOVE KAPPA ENTIRELY - lots changing but leaving name same
    ...
    if(d_lambda != NULL){
        ...
        for(j=0;j<p;j++){
            ...
            for(i=0;i<n;i++){
                ...
                k_i_j = sqrtl( ro[i*p + j] * ro[i*p + j] ); // conditional concentration without univariate kappas
                ...
                fx += (k_i_j + logl(bes_kij) ) - ( S[i*p + j] * ro[i*p + j] );
